...
### 3.1 TC-API-001 — Retrieve User List
...
### 3.2 TC-API-002 — Retrieve Single User
...
### 3.3 TC-API-003 — Retrieve Invalid User (Negative Test)
...
### 3.4 TC-API-004 — Create New User
...
### 4.1 TC-API-005 — Create User and Save ID (Chained)
...
### 4.2 TC-API-006 — Retrieve User by Chained ID
...
### 4.3 TC-API-007 — Delete User by Chained ID
...
### 6.1 TC-API-008 — GET Health Check
...
### 6.2 TC-API-009 — GET List All Users
...
### 6.3 TC-API-010 — POST Create User (Data-Driven)
...
### 6.4 TC-API-011 — GET User by ID (Chained)
...
### 6.5 TC-API-012 — PATCH Partial Update (Chained)
...
### 6.6 TC-API-013 — PUT Replace User (Chained)
...
### 6.7 TC-API-014 — DELETE User (Chained)
...
### 6.8 TC-API-015 — GET Non-Existent User (404)
...
### 6.9 TC-API-016 — POST Validation Error (422)
...
| TC-CI-004 | JSONPlaceholder collection passes | Collection and environment JSON files are committed | Newman runs JSONPlaceholder collection | All assertions pass; exit code 0; pipeline stays green | ⏸ Deferred — JSONPlaceholder and DummyJSON collections were superseded by the QA Live API |
| TC-CI-005 | DummyJSON chained collection passes | Collection and environment JSON files are committed | Newman runs DummyJSON collection | All assertions pass; exit code 0; pipeline stays green | ⏸ Deferred — JSONPlaceholder and DummyJSON collections were superseded by the QA Live API |
| TC-CI-006 | QA Live API collection passes (data-driven) | Collection, environment, and `data/test-data.json` are committed | Newman runs QA Live API collection with `--iteration-data` | All 87 assertions pass; exit code 0; pipeline stays green | ✅ Pass — 87/87 assertions, 3 iterations |
| TC-CI-007 | HTML report uploaded as artifact | Newman runs with htmlextra reporter | Workflow completes | HTML report visible under Artifacts in the GitHub Actions run summary | ✅ Pass — artifact uploaded, 68KB |
| TC-CI-008 | Pipeline fails on assertion failure | A deliberate failing assertion is introduced | Push commit with broken assertion | Workflow run shows ❌; failed step and assertion message visible in logs | ✅ Pass — pipeline failed as expected on intentional assertion error |
| TC-CI-009 | Pipeline recovers after fix | TC-CI-008 has been executed | Revert the breaking change and push | Workflow run shows ✅; all steps green | ✅ Pass — pipeline recovered after fix |

### 8.1 CI Pipeline Evidence

| TC ID | Evidence File |
|---|---|
| TC-CI-008 | `evidence/week6-day4-TC-CI-008-pipeline-failure.png` |
| TC-CI-008 | `evidence/week6-day4-TC-CI-008-failed-step.png` |
| TC-CI-009 | `evidence/week6-day4-TC-CI-009-pipeline-recovery.png` |